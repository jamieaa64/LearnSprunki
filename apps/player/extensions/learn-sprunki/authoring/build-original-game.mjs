...
const sourceRoot = resolve(repositoryRoot, "apps/player/extensions/learn-sprunki/source-assets/sprunkis");
...
for (const [id, name, color, instrumentId, effectId, emoji] of characters) {
  ...
  for (const phaseNumber of [1, 2]) {
    ...
    const sourceAudioRelative = `apps/player/extensions/learn-sprunki/source-assets/sprunkis/${id}/sounds/${phaseId}/${phasePlan.sourceAudio}`;
    lessonTracks.push({
      id: trackId,
